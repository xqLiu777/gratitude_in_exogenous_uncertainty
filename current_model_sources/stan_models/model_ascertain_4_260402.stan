...
transformed parameters {
  ...
  vector[S] slo = exp(mu_slo + sigma_slo * z_slo);
}

model {
  /* ---------- 群体水平先验 ---------- */
  // 截距先验 - 分两侧
  mu_int_neg ~ normal(0, 50);        // 根据y尺度调整
  ...
  // subject loop and trial loop
  {
    ...
    for (n in 1:N) {
      ...
      if (x[n] < 0) {
        mu[n] = int_neg[s] + slo[s] * x[n];
      } else {
        mu[n] = int_pos[s] + slo[s] * x[n];
      }
    }
...
generated quantities {
  ...
  
  {
    ...
    for (n in 1:N) {
      ...
      mu_gen = (x[n] < 0) ? (int_neg[s] + slo[s] * x[n]) : 
                            (int_pos[s] + slo[s] * x[n]);
