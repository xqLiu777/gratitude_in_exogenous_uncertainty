...
model {
  ...
  // 斜率先验 - 分两侧
  mu_slo_neg ~ normal(0, 1);          // x已标准化
  mu_slo_pos ~ normal(0, 1);
  
  /* ---------- 变异参数先验 ---------- */
  // 群体层标准差 - 分两侧
  sigma_int ~ normal(0, 15);
  sigma_int_bias ~ normal(0, 15);        // 根据y尺度调整
  sigma_slo_neg ~ exponential(1);
  sigma_slo_pos ~ exponential(1);
  ...
  
  {
    ...
    for (n in 1:N) {
      int s = subject[n];
      if (x[n] < 0) {
        mu[n] = slo_neg[s] * x[n] - inter[s];
      } else {
        mu[n] = slo_pos[s] * x[n] + inter[s] + inter_bias[s];
      }
    }
...
generated quantities {
  ...
  
  {
    ...
    for (n in 1:N) {
      ...
      // 根据 x 的正负，套用不同的参数
      if (x[n] < 0) {
        mu_gen = slo_neg[s] * x[n] - inter[s];
      } else {
        mu_gen = slo_pos[s] * x[n] + inter[s] + inter_bias[s];
      }
